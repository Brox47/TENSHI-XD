...
const text = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      await m.React('🕘');
      await m.reply('A moment, *TENSHI-XD* is generating your lyrics request...');
      ...
      if (result && result.lyrics) {
        ...
        let buttons = [{
          ...
          },
          {
            name: "cta_url",
            buttonParamsJson: JSON.stringify({
              display_text: "sʜᴏᴡ 💜 ғᴏʀ TENSHI-XD",
              url: `https://whatsapp.com/channel/0029VayazqnEQIaxZoTQZZ1I`
            })
          },
          {
            name: "quick_reply",
